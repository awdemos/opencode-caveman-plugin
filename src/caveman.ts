...
export const CavemanPlugin: Plugin = async (ctx) => {
  ...
  const cavemanCfg = (config.caveman ??= {}) as Partial<CavemanConfig>
  ...
  console.log(
    `[caveman] ${project.id}: enabled=${cavemanCfg.enabled}, intensity=${cavemanCfg.intensity}`,
  )
  ...
  return {
    event: async ({ event }) => {
      ...
      const { message } = event.data as { message: { content?: string; role?: string } }
      const content = (message?.content || '').trim()

      if (!content.startsWith('@caveman')) return
